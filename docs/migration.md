...
| `'Benutzerdefiniert'` as the namespace default | the virtual custom profile (`active_profile_id: __custom__`) |
| `script.portasplit_wohnzimmer_preset_apply` | `climate_profiles.apply_profile` |
...
2. Compare: with the package still active, both the old sensor and
   `sensor.<name>_klimaprofil` should show the same profile. If they do not,
