...
def test_normalise_value(key, raw, expected, vocab):
    assert normalise_value(vocab.get(key), raw) == expected
...
def test_values_equal_uses_half_a_step_as_tolerance(vocab):
    spec = vocab.get("temperature")
    assert values_equal(spec, 24.0, 23.9)
    assert not values_equal(spec, 24.0, 23.0)
    assert not values_equal(spec, 24.0, None)


# --- matching --------------------------------------------------------------


def test_exact_profile_matches(profiles, komfort_state, vocab):
    assert resolve_active_profile(profiles, komfort_state, vocab).name == "Komfort"


def test_one_manual_change_falls_back_to_custom(profiles, komfort_state, vocab):
    komfort_state["temperature"] = 23
    assert resolve_active_profile(profiles, komfort_state, vocab) is None


def test_partial_profile_ignores_undefined_keys(vocab, komfort_state):
    """A = 1, B = 2 matches A = 1, B = 2, C = 999, D = 123."""
    partial = profile("Kuehlung", {"hvac_mode": "cool", "temperature": 24})
    assert profile_matches(partial, komfort_state, vocab)

    komfort_state |= {"fan_mode": "high", "swing_mode": "both", "fan": 7}
    assert profile_matches(partial, komfort_state, vocab)


def test_empty_profile_never_matches(komfort_state, vocab):
    assert not profile_matches(profile("Leer", {}), komfort_state, vocab)


def test_missing_value_prevents_a_match(profiles, komfort_state, vocab):
    """An unavailable entity must not silently count as a match."""
    komfort_state["silent"] = "unavailable"
    assert resolve_active_profile(profiles, komfort_state, vocab) is None


def test_off_profile_matches_although_the_rest_is_stale(profiles, vocab):
...
def test_case_and_type_differences_do_not_break_matching(profiles, vocab):
    state = {
        "hvac_mode": "COOL",
        "temperature": "24",
        "swing_mode": "Off",
    ...
    assert resolve_active_profile(profiles, state, vocab).name == "Komfort"


def test_no_profiles_means_custom(komfort_state, vocab):
    assert resolve_active_profile(ProfileSet(), komfort_state, vocab) is None
